...
- For episode artwork, preserve the priority order remote artwork, existing local sidecar/NFO reference, season or series preview fallback, then an ffmpeg screenshot fallback. A known remote preview must not trigger video capture merely because its local download failed. Generate screenshots only during a confirmed NFO operation when no usable preview exists, never overwrite existing artwork, skip non-regular media, and report per-file failures in the result.
